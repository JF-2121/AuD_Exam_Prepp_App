...
- Per-device local storage only (IndexedDB) — there is no sync across devices; switching phone/tablet/laptop means separate progress. This is current architecture, not yet confirmed as a permanent constraint if multi-device sync is ever desired.
