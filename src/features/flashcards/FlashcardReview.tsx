    ...
    <div>
      <div className="mb-4 flex items-center gap-2">
        <h1 className="flex items-center gap-2 text-2xl font-semibold tracking-tight text-[var(--color-text-h)]">
          <SquareStack size={22} className="text-[var(--color-accent)]" /> {t('flash.title')}
        </h1>
        <select
      ...
      {!loaded ? (
        <p className="text-[var(--color-text-dim)]">{t('app.loading')}</p>
      ) : !card ? (
